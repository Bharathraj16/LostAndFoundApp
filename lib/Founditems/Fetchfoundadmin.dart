...
class _FetchAdminState extends State<FetchAdmin> {
  ...
  late CollectionReference Found_Items;
  ...
  void initState() {
    ...
    Found_Items = FirebaseFirestore.instance.collection('Found_Items');
  }
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    ...
                    if (category.isNotEmpty ||
                        ...
                        location.isNotEmpty) {
                      await Found_Items.doc(documentSnapshot!.id).update({
                        "Category": category,
                        "Item Name": name,
                        "Found Date": date,
                        "Location": location,
  ...
  Future<void> delete(String Lost_itemsId) async {
    await Found_Items.doc(Lost_itemsId).delete();
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: StreamBuilder<QuerySnapshot>(
          // Filter items by current user's UID
          stream: Found_Items.where('userId', isEqualTo: currentUserId)
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 10),
                        _buildDetailRow("Found Within:", document['Location']),
                        const SizedBox(height: 10),
                        _buildDetailRow("Found Date:", document['Found Date']),
                        const SizedBox(height: 20),
                        Row(
